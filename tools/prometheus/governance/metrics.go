...
func getAllInactiveProposalsID(cdc *codec.Codec, ctx context.CLIContext) (proposals gov.ProposalQueue, err error) {
	if res, err := ctx.QueryStore(gov.KeyInactiveProposalQueueProposal(time.Now(),1), storeName); err != nil {
...
func getAllActiveProposalsID(cdc *codec.Codec, ctx context.CLIContext) (proposals gov.ProposalQueue, err error) {
	if res, err := ctx.QueryStore(gov.KeyActiveProposalQueueProposal(time.Now(),1), storeName); len(res) == 0 || err != nil {
		return gov.ProposalQueue{}, err
	} else {
		err = cdc.UnmarshalBinaryBare(res, &proposals)
		return proposals, err
...
func getProposal(ID uint64, cdc *codec.Codec, ctx context.CLIContext) (*gov.Proposal, error) {
	if res, err := ctx.QueryStore(gov.KeyProposal(ID), storeName); err != nil {
		return nil, err
	} else {
		var proposal *gov.Proposal
		err = cdc.UnmarshalBinaryBare(res, proposal)
		return proposal, err
	}
}

func getVote(proposalID uint64, voterAddr sdk.AccAddress, cdc *codec.Codec, ctx context.CLIContext) (vote gov.Vote, err error) {
	if res, err := ctx.QueryStore(gov.KeyVote(proposalID, voterAddr), storeName); err != nil {
		...
		err = cdc.UnmarshalBinaryBare(res, &vote)
		return vote, err
	}
}
